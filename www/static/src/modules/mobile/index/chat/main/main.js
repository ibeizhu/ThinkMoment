    ...
    methods:{
        ...
        bindPusher:function () {
            ...
            channel.bind('moment-push', function(data) {
                var index = self.getIndex(self.session.messages,data.virtualChatId);
                if(index > -1){
                    self.session.messages[index].sending = false;
                    self.session.messages[index].chatId = data.chatId;
                }else{
